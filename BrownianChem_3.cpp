...
int main ( int argc, char *argv[] )
{
  ...
  double delRij[3],delRij2,sqRij,Eij,hFi[3], totEnergy, totSqrij;
  double *Fij;
  double rand1,rand2,RB,randhBr[3];
...
//Read partickle tipes
  ...
  for(int i=0; i < particklTipesNum; i++)
  {
    modelFile>>
      ...
      particklTipes[i].radius>>
      particklTipes[i].mass>>
      particklTipes[i].viscosityPar>>
      particklTipes[i].randForceScale;
  }
...
//Time step, Number of steps, Data collection number of steps, equilibration
  ...
  ofstream xyzFile( xyzFileName.c_str() ); 
  ...
  for(int i=0; i<particklTipesNum; i++)
  {
    for(int j=0; j<particklTipes[i].maxNumber; j++)
    {
      ...
      if(j<particklTipes[i].initNumber)
      {
        ...
        for(int k=0; k<3; k++)
        {
          partickles[l].coord[k]=boxSize[k]*rand()/((double)RAND_MAX);
              ...
              for(int m=0; m<reactionsNumber; m++)
              {
                        ...
                        for(int p=0; p<totalParticklNumber; p++)                       
                        {
                          if(partickles[p].existence ==0 && partickles[p].partTypeID==reactions[m].products[n])
                          {
                            ...
                            for(int k=0; k<3; k++)
                            {
                              ...
                              RB=particklTipes[partickles[p].partTypeID].randForceScale;
                ...
                {                
                  ...
                  {    
                    rand1=rand()/((double)RAND_MAX);
                    rand2=rand()/((double)RAND_MAX);
                    RB=particklTipes[partickles[i].partTypeID].randForceScale;
                    randhBr[k]=pow( -2.*(2.*delTime*RB)*log(rand1+0.000000001) , 0.5 ) * cos(2.*PI*rand2);
    ...
    if(step > relaxStepsNumber)
    {
...

      {
        ...
        for(int i=0; i<totalParticklNumber; i++)
        {
          ...
          {
            existed++;
						sstream << particklTipes[partickles[i].partTypeID].name << " "
            << partickles[i].coord[0]<<" "<<partickles[i].coord[1]<<" "<<partickles[i].coord[2]<<"\n";
            xyzFileCoords = xyzFileCoords + sstream.str();
          } 
...
// Lennard-Jones potential energy  
  ...
  double eps=parameters[1];
  double sigma=parameters[2];
  double rcoff=parameters[0];
...
{ 
  ...
  if(Eij>Energy && rand()/((double)RAND_MAX)>probability)
  {
    return(1);
  }
  else return(0);  
